...
async def list_products(
    ...
    try:
        ...
        products = await product_manager.fetch_all(filters=filters)
        
        # Apply search filter if provided
        if search:
            search_lower = search.lower()
            filtered_items = [
                p for p in products.items 
                if search_lower in p.product_name.lower() or 
                   search_lower in p.sku.lower()
            ]
            products.items = filtered_items
            products.count = len(filtered_items)
...
async def deactivate_product(
    ...
    try:
        await product_manager.update(product_id, {"is_active": False})
        return StatusResponse(message="Product deactivated successfully")
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"Failed to deactivate product: {str(e)}"
