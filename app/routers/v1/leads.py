...
async def call_pivot_report(
    ...
    """Call pivot, BOTH directions = the 5-row daily funnel (2026-07-17) — Incoming/
    Outgoing Calls / Fresh Leads / Deduped / Connected / Orders Booked (see
    crmReportService._call_funnel); the Task C1 stage x call-date pivot is gone.
    Agency-scoped for agency admins via _report_scope_owner. Registered before
    /{lead_id} so 'call-pivot' is not swallowed by the lead-detail route."""
